...
class FeesController extends Controller
{
    ...
    {
        $fees = $this->fees->where('status', 1)->orderBy('id', 'desc')->get();   
    ...
    public function create($id)
    {
        ...
        return view('feesadd', compact('customers'));
    }
    ...
    public function store(Request $request)
    {
        ...
        // Check if the customer has already paid fees for the selected month
        $existingFees = fees::where('customerid', $validatedData['customerid'])
            ->where('month', $validatedData['month'])
            ->first();

        if ($existingFees) {
            // Redirect back with an error message
            return redirect()->back()->with('error', 'Fees for the selected month have already been added for this customer.');
        }
    ...
    public function newfees(Request $request)
    {
        // Validate the request
        $validatedData = $request->validate([
            'name' => 'required',
        ...
        if ($customer) {
            ...
            return redirect()->back()->with('success', 'Customer Fees Added and Due Date Updated successfully.');
        } else {
            // If no matching customer exists, handle it
            return redirect()->back()->with('error', 'Customer not found in the database.');
        }
    ...
    public function income_report(Request $request)
        {
            ...
            try {
                $startDate = Carbon::parse($request->start_date)->startOfDay(); // Start at 00:00:00
                $endDate = Carbon::parse($request->end_date)->endOfDay(); // Ensure it covers the whole day (23:59:59)

                // Retrieve data from the Fees model based on the date range
                $fees = Fees::whereBetween('date', [$startDate, $endDate])
                            ->where('status', 1)
                            ->get();
    ...
    public function invoice($id)
    {
        ...
        // Return the invoice view with the customer data
        return view('invoice', compact('customer'));
    }
